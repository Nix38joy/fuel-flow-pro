...
export interface FuelFlowData {
  fuels: Record<FuelId, Fuel>;
  pumps: Pump[];
  orders: Order[];
}

const initialFuels: Record<FuelId, Fuel> = {
...
export const createInitialFuelFlowData = (): FuelFlowData => ({
  fuels: Object.fromEntries(
    Object.entries(initialFuels).map(([id, fuel]) => [id, { ...fuel }])
  ) as Record<FuelId, Fuel>,
  pumps: initialPumps.map((pump) => ({ ...pump })),
  orders: [],
});
